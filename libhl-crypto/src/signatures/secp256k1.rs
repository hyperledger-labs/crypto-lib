...
pub struct EcdsaSecp256K1Sha256PublicKey {
    value: ecdsa_secp256k1_sha256::EcdsaSecp256K1Sha256PublicKeyImpl
}

impl EcdsaSecp256K1Sha256PublicKey {
    pub fn as_slice(&self) -> [u8; PUBLIC_KEY_SIZE] { self.value.as_slice() }
    pub fn as_hex(&self) -> String { self.value.as_hex() }
    pub fn as_uncompressed_slice(&self) -> [u8; PUBLIC_UNCOMPRESSED_KEY_SIZE] { self.value.as_uncompressed_slice() }
    pub fn as_uncompressed_hex(&self) -> String { self.value.as_uncompressed_hex() }
...
impl Clone for EcdsaSecp256K1Sha256PublicKey {
    fn clone(&self) -> EcdsaSecp256K1Sha256PublicKey {
        EcdsaSecp256K1Sha256PublicKey { value: self.value.clone() }
    }
}

impl fmt::Display for EcdsaSecp256K1Sha256PublicKey {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        write!(formatter, "EcdsaSecp256K1Sha256PublicKey {{ value: {} }}", self.value)
    }
}
...
impl PublicKey for EcdsaSecp256K1Sha256PublicKey {
    ...
    fn verify(&self, message: &[u8], signature: &[u8]) -> Result<bool, CryptoError> {
        self.value.verify(message, signature)
    }
}

pub struct EcdsaSecp256K1Sha256PrivateKey {
    value: ecdsa_secp256k1_sha256::EcdsaSecp256K1Sha256PrivateKeyImpl
}
...
impl EcdsaSecp256K1Sha256PrivateKey {
    ...
    pub fn as_slice(&self) -> [u8; PRIVATE_KEY_SIZE] { self.value.as_slice() }
...
impl Clone for EcdsaSecp256K1Sha256PrivateKey {
    fn clone(&self) -> EcdsaSecp256K1Sha256PrivateKey {
        EcdsaSecp256K1Sha256PrivateKey { value: self.value.clone()}
    }
}

impl fmt::Display for EcdsaSecp256K1Sha256PrivateKey {
    fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        write!(formatter, "EcdsaSecp256K1Sha256PrivateKey {{ value: {} }}", self.value)
    }
}
...
impl PrivateKey<EcdsaSecp256K1Sha256PublicKey> for EcdsaSecp256K1Sha256PrivateKey {
    ...
    fn sign(&self, message: &[u8]) -> Result<Vec<u8>, CryptoError> {
        self.value.sign(message)
    }

    fn get_public_key(&self) -> EcdsaSecp256K1Sha256PublicKey {
        let value = self.value.get_public_key();
...
mod ecdsa_secp256k1_sha256 {
    ...
    impl EcdsaSecp256K1Sha256PublicKeyImpl {
        pub fn as_slice(&self) -> [u8; PUBLIC_KEY_SIZE] { self.value }
        pub fn as_hex(&self) -> String {
            bin2hex(&self.as_slice()[..])
        }
        pub fn as_uncompressed_slice(&self) -> [u8; PUBLIC_UNCOMPRESSED_KEY_SIZE] {
    ...
    impl fmt::Display for EcdsaSecp256K1Sha256PublicKeyImpl {
        fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
            write!(formatter, "EcdsaSecp256K1Sha256PublicKeyImpl {{ value: {} }}", bin2hex(&self.value[..]))
        }
    }
    ...
    impl EcdsaSecp256K1Sha256PrivateKeyImpl {
        ...
        pub fn as_slice(&self) -> [u8; PRIVATE_KEY_SIZE] { self.value }
        pub fn as_hex(&self) -> String { bin2hex(&self.as_slice()[..]) }
        pub fn from_slice(data: &[u8]) -> Result<EcdsaSecp256K1Sha256PrivateKeyImpl, CryptoError> {
        ...
        pub fn get_public_key(&self) -> EcdsaSecp256K1Sha256PublicKeyImpl {
            ...
            zero!(s);
            EcdsaSecp256K1Sha256PublicKeyImpl { value: w }
        }
    }
    ...
    impl fmt::Display for EcdsaSecp256K1Sha256PrivateKeyImpl {
        fn fmt(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
            write!(formatter, "EcdsaSecp256K1Sha256PrivateKeyImpl {{ value: {} }}", bin2hex(&self.value[..]))
        }
    }
