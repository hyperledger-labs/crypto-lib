...
use hl_crypto::signatures::{PublicKey, PrivateKey, SignatureScheme, EcdsaPublicKeyHandler, sha256};
...
fn main() {
    ...
    let sk = secp256k1::key::SecretKey::from_slice(&context, &s[..]).unwrap();
    let pk = secp256k1::key::PublicKey::from_slice(&context, &p[..]).unwrap();
